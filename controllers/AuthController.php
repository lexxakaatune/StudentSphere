        ...
        // ERROR HANDLERS
          ...
          if (!empty($email) && !empty($pwd)) {
            ...
            if (!empty($pwdPass)) { 
              ...
              header('Location: ?page=auth&action=profile');
              exit;
            } else {
            ...
            $error = "Fill in all fields!";
            set_flash('login_error', $error);
            header('Location: ?page=auth&view=login&error=notPost');
            exit;
          }
      } catch (PDOException $e) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      try {
        ...
        if (isset($_SESSION['flash']['user-id'])) {
          ...
          UserModel::deleteUser($email, $dBPwd);
          header("Location: ?page=auth&action=admin&view=students");
          exit;
        } 
      } catch (PDOException $e) {
        die("Query failed: " . $e->getMessage());
      ...
      try {
        ...
        if (!empty($record)) {
          ...
          if ($pass) {
            ...
            header("Location: ?page=auth&action=admin&view=overviews");
            exit;
          } else {
            $error = 'Wrong Password';
